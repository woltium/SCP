...
def objective(opt_vars,
    ...
    opt_pose_quat_wxyz = T.euler_to_quat(opt_pose[3:])
    opt_pose_homo = T.pose_to_mat([opt_pose[:3], T.euler_to_quat(opt_pose[3:])])

    cost = 0
    # collision cost
    if collision_points_centered is not None:
        collision_cost = 30 * calculate_collision_cost(opt_pose_homo[None], sdf_func, collision_points_centered, 0.005)
        debug_dict['collision_cost'] = collision_cost
        cost += collision_cost

    # stay close to initial pose
    init_pose_cost = 5.0 * consistency(opt_pose_homo[None], init_pose_homo[None], rot_weight=2)
    ...
    max_iterations = 10
    opt_pose_wxyz =  np.concatenate((opt_pose[:3],opt_pose_quat_wxyz))
    ...
    if is_grasp_stage:
        if preferred_grasp_pose is not None:
            grasp_cost = consistency(opt_pose_homo[None], preferred_grasp_pose[None], rot_weight=0.5)
        else:
...
class SubgoalSolver:
    ...
    def _warmup(self):
        ee_pose = np.array([0.0, 0.0, 0.3, 1, 0, 0, 0])
        ...
        self.solve(ee_pose, keypoints, keypoint_movable_mask, goal_constraints, path_constraints, sdf_voxels, collision_points, True, None,None,from_scratch=True)
        self.last_opt_result = None
    ...
    def solve(self,
        ...
        ee_pose_homo = T.pose_to_mat([ee_pose[:3], ee_pose[3:]])
        #print ee_pose_homo and preferred_grasp_pose_homo:
        ee_pose_euler = np.concatenate([ee_pose[:3], T.quat_to_euler(ee_pose[3:])])
        ...
        print('subgoal_solver: solve time:', solve_time)
        # ====================================
        ...
        sol = np.concatenate([sol[:3], T.euler_to_quat(sol[3:])])
        #sol pos = preferred_grasp_pos:
        # if preferred_grasp_pose is not None:
        #     sol[:3] = preferred_grasp_pose[:3,3]
        #     sol[3:] = T.mat_to_quat(preferred_grasp_pose[:3][:3])
