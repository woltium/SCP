...
def objective(opt_vars,
    ...
    if collision_points_centered is not None:
        collision_cost = 10 * calculate_collision_cost(poses_homo[start_idx:end_idx], sdf_func, collision_points_centered, 0.01)
        #global totaltime
    ...
    path_length_cost = 2 * approx_length
    ...
    max_iterations = 20
    start_time = time.time()
    for control_point_quat in control_points_quat:
       # Extract position (x, y, z) and quaternion (xyzw) using slicing
        position = control_point_quat[:3]  # x, y, z
        quat_wxyz = control_point_quat[3:] # qw, qx,qy, qz

        # Reorder quaternion to wxyz using slicing
        target_pose =np.concatenate([position, quat_wxyz])
        ik_result = ik_solver.solve(
                        target_pose,  # Pass wxyz order using unpacking
...
class PathSolver:
    ...
    def _warmup(self):
        start_pose = np.array([0.0, 0.0, 0.4, 1, 0, 0, 1])
        end_pose = np.array([0.0, 0.0, 0.1, 1, 0, 0, 1])
    ...
    def _center_collision_points_and_keypoints(self, ee_pose, collision_points, keypoints, keypoint_movable_mask):
        ee_pose_homo = T.pose_to_mat([ee_pose[:3], T.euler_to_quat(ee_pose[3:])])
    ...
    def solve(self,
        ...
        """
        Args:
            - start_pose (np.ndarray): [7], [x, y, z, qw, qx, qy, qz]
            - end_pose (np.ndarray): [7], [x, y, z, qw, qx, qy, qz]
        ...
        num_control_points = np.clip(num_control_points, 4, 6)
        # transform to euler representation
        start_pose = np.concatenate([start_pose[:3], T.quat_to_euler(start_pose[3:])])
        end_pose = np.concatenate([end_pose[:3], T.quat_to_euler(end_pose[3:])])
        ...
        #print('shape of interp_poses', interp_poses.shape)
        interp_poses_homo = T.convert_pose_euler2mat(interp_poses)
        init_collision_cost = 150 * calculate_collision_cost(interp_poses_homo[1:-1], sdf_func, collision_points_centered, 0.005)
        #print('init_collision_cost', init_collision_cost)
        if init_collision_cost>5:
            print('add more control points')
        ...
        # initial guess as linear interpolation
            ...
            # for i in range(len(init_sol), num_vars, 6):
            #     new_x0[i:i+6] = init_sol[-6:] + np.random.randn(6) * 0.05
        ...
        # ====================================
        start = time.time()
        optimization_context = type('OptContext', (), {})()
        # use global optimization for the first iteration
        maxfun=self.config['sampling_maxfun']
        if init_collision_cost ==0:
            print('no collision,cut down maxfun')
            maxfun = self.config['sampling_maxfun']/2
        if from_scratch:
        ...
        # Add cost history to debug_dict
        #debug_dict['cost_history'] = optimization_context.costs
        solve_time = time.time() - start
        #print("path_solver_solve_time: ", solve_time)
        # ====================================
        ...
        print("path_solver_total_time",totaltime)
