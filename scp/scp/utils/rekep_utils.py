...
def calculate_collision_cost(poses, sdf_func, collision_points, threshold, threshold_multiplier=5):
    ...
    # First region: within threshold
    region1_mask = (signed_distance + threshold > 0)
    cost1 = np.sum((signed_distance + threshold)[region1_mask])
    
    # Second region: between threshold and threshold*5
    region2_mask = (signed_distance + threshold_multiplier*threshold >0) & (signed_distance + threshold < 0)
    cost2 = 1/(threshold_multiplier*2) * np.sum((signed_distance + threshold_multiplier*threshold)[region2_mask])
    #cost2 =0
    collision_cost = cost1 + cost2
...
def get_callable_grasping_cost_fn(env):
    def get_grasping_cost(keypoint_idx):
        ...
        return -env.is_grasping(candidate_obj_id=keypoint_object) + 1  # return 0 if grasping an object, 1 if not grasping any object
...
def linear_interpolate_poses(start_pose, end_pose, num_poses):
    ...
    if start_pose.shape == (6,) and end_pose.shape == (6,):
        ...
        start_rotmat = T.euler_to_rmat(start_euler)
        end_rotmat = T.euler_to_rmat(end_euler)
        ...
        start_rotmat = T.quat_to_rmat(start_quat)
        end_pos, end_quat = end_pose[:3], end_pose[3:]
        end_rotmat = T.quat_to_rmat(end_quat)
    else:
...
def spline_interpolate_poses(control_points, num_steps):
    if np.any(np.isnan(control_points)) or np.any(np.isinf(control_points)):
        print("Error: control_points contains NaN or inf!")
        print(control_points)
        raise ValueError("Invalid input: control_points contains NaN or inf")
    ...
    if control_points.shape[1] == 6:
        ...
        control_points_euler = control_points[:, 3:]
        control_points_rotmat = []
        for euler in control_points_euler:
            rmat = T.euler_to_rmat(euler)
            # 检查旋转矩阵有效性
            if np.any(np.isnan(rmat)) or np.any(np.isinf(rmat)):
                raise ValueError("Invalid rotation matrix from euler angles")
            control_points_rotmat.append(rmat)
        control_points_rotmat = np.array(control_points_rotmat)
    elif control_points.shape[1] == 4 and control_points.shape[2] == 4:
        control_points_pos = control_points[:, :3, 3]
        control_points_rotmat = control_points[:, :3, :3]
        # 检查旋转矩阵正交性和行列式
        for rmat in control_points_rotmat:
            if not np.allclose(rmat @ rmat.T, np.eye(3), atol=1e-6):
                raise ValueError("Rotation matrix not orthogonal")
            det = np.linalg.det(rmat)
            if not np.isclose(det, 1.0, atol=1e-6):
                raise ValueError(f"Rotation matrix determinant is {det}, not 1")
    elif control_points.shape[1] == 7:
        control_points_pos = control_points[:, :3]
        control_points_rotmat = []
        for quat in control_points[:, 3:]:
            rmat = T.quat_to_rmat(quat)
            if np.any(np.isnan(rmat)) or np.any(np.isinf(rmat)):
                raise ValueError("Invalid rotation matrix from quaternion")
            control_points_rotmat.append(rmat)
        control_points_rotmat = np.array(control_points_rotmat)
    else:
        raise ValueError('Unsupported control_points format')
    ...
    if len(control_points_pos) < 2:
        raise ValueError("Insufficient control points after filtering")

    # 生成时间参数（关键修改）
    times = np.linspace(0, 1, len(control_points_pos))  # 归一化时间参数
    ...
    try:
        ...
        print(f"Error creating splines: {e}")
        print("Control points_pos:", control_points_pos)
        print("Times:", times)
        print("Rotation matrices:", control_points_rotmat)
        raise

    # 采样插值结果
    pos_samples = sample_from_spline(pos_spline, num_steps)
    rot_samples = sample_from_spline(rot_spline, num_steps)

    # 重构输出格式
    if control_points.shape[1] == 6:
        return np.array([np.concatenate([p, T.rmat_to_euler(r)]) 
                       for p, r in zip(pos_samples, rot_samples)])
    elif control_points.shape[1] == 4 and control_points.shape[2] == 4:
        poses = np.tile(np.eye(4), (num_steps, 1, 1))
        poses[:, :3, :3] = rot_samples
        poses[:, :3, 3] = pos_samples
        return poses
    elif control_points.shape[1] == 7:
        return np.array([np.concatenate([p, T.mat_to_quat(r)]) 
                       for p, r in zip(pos_samples, rot_samples)])
    else:
        raise ValueError('Unsupported output format')
def spline_interpolate_poses_(control_points, num_steps):
    """
    ...
    if control_points.shape[1] == 6:
        ...
        for control_point_euler in control_points_euler:
            control_points_rotmat.append(T.euler_to_rmat(control_point_euler))
        ...
        for control_point_quat in control_points[:, 3:]:
            control_points_rotmat.append(T.quat_to_rmat(control_point_quat))
        control_points_rotmat = np.array(control_points_rotmat)
    else:
    ...
    if control_points.shape[1] == 6:
        ...
        for i in range(num_steps):
            pose = np.concatenate([pos_samples[i], T.rmat_to_euler(rot_samples[i])])
            poses.append(pose)
        ...
        for i in range(num_steps):
            quat = T.mat_to_quat(rot_samples[i])
