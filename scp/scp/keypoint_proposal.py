...
from scp.utils.rekep_utils import filter_points_by_bounds
...
class KeypointProposer:
    ...
    def get_keypoints(self, rgb, points, masks,task_prompt = None):
        # get features
        if self.feature_type == 'dino':
        ...
        print('candidata_keypoints:', candidate_keypoints.shape)
    ...
    def _get_features(self, transformed_rgb, shape_info, text_prompt=None):
        if self.feature_type == 'dino':
            return self._get_dino_features(transformed_rgb, shape_info)

        
    def _get_dino_features(self, transformed_rgb, shape_info):
    ...
    def _cluster_features(self, points, features_flat, masks):
        ...
        for rigid_group_id, binary_mask in enumerate(masks):
            ...
            ##wws
            if len(obj_features_flat) < self.config['num_candidates_per_mask']:
                continue
