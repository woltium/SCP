            ...
            for j in range(len(seq)):
                ...
                if write_video:
                    if video_count % video_skip == 0:
                        video_img = []
                        if robokep_interface is not None:
                            obs = robokep_interface.get_cam_obs()
                        for cam_name in camera_names:
                            if robokep_interface is not None:
                                im = robokep_interface.env.sim.render(height=512, width=512, camera_name=cam_name)
                                video_img.append(im[::-1])
                            else:
                                origin = env.render(mode="rgb_array", height=512, width=512, camera_name=cam_name)
                                video_img.append(origin)
                ...
                waypoint_quat = T.axisangle2quat(action_pose[3:])
                # from xyzw to wxyz:
                waypoint_quat = np.concatenate([waypoint_quat[:1], waypoint_quat[1:]])
                waypoint_quat_pose = np.concatenate([action_pose[:3], waypoint_quat])
                # add in gripper action
                play_action = np.concatenate([action_pose, waypoint.gripper_action], axis=0)
                print
                if waypoint.gripper_action == 1 and last_gripper_action ==-1:
                    #robokep_interface._move_to_waypoint(waypoint_quat_pose)
                    for i in range(5):
                        env.step(play_action)
                last_gripper_action = waypoint.gripper_action
