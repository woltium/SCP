...
def parse_source_dataset(
    ...
    for ind in tqdm(range(len(demo_keys))):
        ...
        ep_datagen_info_kwargs = { # 使用字典来构建参数
        ...
        # 检查 expert_frame_segments 是否存在
        if "expert_frame_segments" in ep_datagen_info:
            ep_datagen_info_kwargs["expert_frame_segments"] = ep_datagen_info["expert_frame_segments"][:]

        ep_datagen_info_obj = DatagenInfo(**ep_datagen_info_kwargs) # 使用 **kwargs 传递参数
...
def write_demo_to_hdf5(
    ...
    env_interface_name=None,
    env_interface_type=None,
):
    """
    ...
    if env_interface_name and env_interface_type is not None:
        ep_data_grp["datagen_info"].attrs["env_interface_name"] = env_interface_name
        ep_data_grp["datagen_info"].attrs["env_interface_type"] = env_interface_type
    data_writer.close()
