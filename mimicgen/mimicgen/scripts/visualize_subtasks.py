...
def visualize_subtasks(args):
    ...
    if write_video:
        video_writer.close()
def visualize_subtasks_per_demo(args):
    """
    为每个 demo 单独保存视频的版本。
    """
    # some arg checking
    assert args.video_path is not None, "Must specify a video directory to save per-demo videos"
    write_video = True
    ...
    for ind in range(len(demo_keys)):
        ...
        video_path = os.path.join(video_dir, "video_{}.mp4".format(ep))
    ...
    f.close()        
def save_video(config_path,dataset_path,video_path):
    """
    调用 visualize_subtasks 函数，仅使用 videopath 和 configpath 作为参数。

    Args:
        videopath (str): 视频文件的输出路径。
        configpath (str): 配置文件的路径。
    """
    import argparse

    # 创建一个 argparse.Namespace 对象，模拟命令行参数
    args = argparse.Namespace(
        dataset=dataset_path,  # 需要指定数据集路径
        config=config_path,
        signals=None,
        offsets=None,
        filter_key=None,
    ...
    # 调用 visualize_subtasks 函数
    visualize_subtasks_per_demo(args)
